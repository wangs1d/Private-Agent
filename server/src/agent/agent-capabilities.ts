  ...
  return [
    ...
    },
    {
      ...
      lines: [
        ...
        "9️⃣c search_images 数量与排版：单次默认 4 张、上限 8 张。**不要追求数量**，宁可少而精。多个地点/多个主题请**按主题各发一次 query 并行搜**（每个 query 用完整具体的描述，如「平潭岛蓝眼泪荧光海 夜景实拍」「黄山云海日出 云海翻涌」，不要把单个主题截成碎片短词），由前端 renderBlocks 把「每段文字→对应几张照片」自然交错；不要一次性塞 20+ 张图，也不要把多个不同主题的图全堆在一张图墙里。",
      ],
    },
    {
      domain: "embodiment",
