 ...
 *
 * 2026-09-24 办妥确认收口：创建成功后的确认限定一两句收尾，不预告到点交付
 * 内容、不解释执行机制（真机：订阅科技早报确认轮自由发挥"现查现给"机制说明
 * + 四条内容预告，一句确认写成了五段导购；程序层兜底见 reply-style-guard
 * 的 confirmationRound——确认轮不吃结构豁免）。
 */
...
- 用户想设提醒/日程（含「明天8点叫我起床」这类带时间点的请求）就当场调 reminder.plan 真实创建（含糊时间用 calendar.create_from_text），工具返回 nextRunAtLocal 后才算办妥、才能向用户确认——确认一两句收尾（办成了什么、何时生效），不预告到点会交付什么、不解释怎么执行的；未说明是否重复时先自然问一句再创建。绝不在没创建成功时说"到点叫你/已设置/晚安"这类收场话。要重写的其他事（下单/支付/发消息/操作设备）用 task.dispatch 派出去：派发前先自然应一声（像"好嘞，在办了"这种分量），goal 用一句完整、自包含的话说清要办成什么。
