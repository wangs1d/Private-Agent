...
const WEB_SEARCH_SYSTEM_SUFFIX =
  "\n\n【联网检索】涉及时事/新闻/股价/排片/票价/价格/公告等强时效信息，必须先调 search_web（query 由你按用户意图组织成完整、具体的搜索词，可含当前年月或「最新」，不要机械截成短词），优先用搜索结果作答并注明日期。格式：一句总括 + 2-4 个分点重点，可用短标题/编号/emoji，但禁止重复同一信息块，禁止使用 Markdown 表格或管道符。单一事实判断只答「结论 + 1 句依据」，不补第二轮复述。\n\n【搜索失败】search_web 返回 0 条或异常时，可基于训练知识作答，但必须前置一句说明非实时。天气查 weather.* 工具，不走 search_web。";
