...
环境变量：
  PRIVATE_AI_AGENT_BASE_URL  主服务地址，默认 http://127.0.0.1:${PORT:-3000}
...
def _default_base_url() -> str:
    """优先 PRIVATE_AI_AGENT_BASE_URL，其次根据 PORT 推断，最后 3000。"""
    env = os.environ.get("PRIVATE_AI_AGENT_BASE_URL", "").strip()
    if env:
        return env.rstrip("/")
    port = (os.environ.get("PORT") or "3000").strip() or "3000"
...
class TranslateApiClient:
    ...
    def translate_text(
        ...
        try:
            with httpx.Client(timeout=self.timeout_s) as cli:
                r = cli.post(f"{self.base_url}/api/translate/text", json=payload)
        except Exception as e:
            LOG.exception("文本翻译请求失败")
            return TranslateResult(ok=False, error=f"无法连接主服务: {e}")
        if r.status_code < 200 or r.status_code >= 300:
            return TranslateResult(ok=False, error=f"主服务 HTTP {r.status_code}: {r.text[:200]}")
        try:
        ...
        return TranslateResult(
            ok=True,
            source_text=str(data.get("sourceText") or text),
            translated_text=str(data.get("translatedText") or ""),
            target_lang=str(data.get("targetLang") or target),
            translated_by=str(data.get("translatedBy") or "none"),
            line_count=0,
